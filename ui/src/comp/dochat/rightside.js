...
class RightPanel extends Component{
    ...
    render(){
      ...
      //const buttonToShow = this.state.showSend ? <Button variant="contained" disabled={this.props.sendButtonDisabled} sx={{height:'100%', fontSize:this.props.fontSizeSM}} onClick={this.onQuery} endIcon={<SendIcon />}>Send</Button> : <Button variant="contained" disabled={this.props.sendButtonDisabled} sx={{height:'100%', fontSize:this.props.fontSizeSM}} onClick={this.onQuery} endIcon={<StopIcon />}>Cancel</Button>;
      ...
      this.props.chatMessages.map((message, index) => {
        //console.log(".....side: %s, text: %s .....", message.side, message.text);
      });
      ...
      return (
        <>
          ...
          >              
              <Box sx={{ display: 'flex', flexDirection: 'column', width: lSplit}}>
                <Box
                  sx={{
                    flexGrow: 1,
                    overflowY: 'auto',
                  ...
                  <Grid container sx={{width: '100%', boxSizing: 'border-box'}} border={0}>
                    <Grid item xs={12} sx={{boxSizing: 'border-box'}} border={0}>
                      <Grid container direction='row' width='100%' border={0}>
                        <Grid item sm={8} xs={12} pr={2} py={0.2} border={0}>
                          <input type='text' 
                        ...
                        <Grid item display='flex' justifyContent='center' sm={2} xs={12} py={0.2} border={0} sx={{boxSizing: 'border-box'}} >
                        {buttonToShow}
                        </Grid>
              ...
              <Box sx={{display: 'flex', flexDirection: 'column', height: this.props.isMobile ? 'auto' : '100%', overflowY: 'none', fontSize:this.props.fontSizeXS, width: rSplit, borderLeft:'1px solid #ccc'}}>
                ...
                <Box display='flex' border={0} flexDirection={'column'} sx={{width:'100%', height: this.props.isMobile ? 'auto' : '100%', overflowY: buttomBoxOverflow}}>
